...
def test_create_deploy_approval(CL_TYPES, DEPLOY_TYPES, DEPLOY_FACTORY, FACTORY, vector_cl_data_1, vector_crypto_2):
    ...
    for algo, pbk, pvk in [operator.itemgetter("algo", "pbk", "pvk")(i) for i in vector_crypto_2]:
        ...
        approval = DEPLOY_FACTORY.create_deploy_approval(account_info, _bytes)
        assert isinstance(approval, DEPLOY_TYPES.DeployApproval)


def test_create_deploy_parameters(CL_TYPES, DEPLOY_TYPES, DEPLOY_FACTORY, FACTORY, a_test_account, a_test_chain_id):
    assert isinstance(
        DEPLOY_FACTORY.create_deploy_parameters(
...
def test_create_transfer_payment(DEPLOY_TYPES, DEPLOY_FACTORY):
    assert isinstance(
        DEPLOY_FACTORY.create_standard_payment(
...
def test_create_transfer_body(DEPLOY_TYPES, DEPLOY_FACTORY, deploy_params):
    payment = DEPLOY_FACTORY.create_standard_payment(
        amount = random.randint(0, 1e5),
        )
    ...
    body = DEPLOY_FACTORY.create_deploy_body(payment, session)
...
def test_create_transfer_header(DEPLOY_TYPES, DEPLOY_FACTORY, deploy_params):
    ...
    body = DEPLOY_FACTORY.create_deploy_body(payment, session)
    header = DEPLOY_FACTORY.create_deploy_header(body, deploy_params)
