...
async def test_get_information_signed_block(
    ...
    # Block ID = None.
    data: Response = \
        await NODE_BINARY_CLIENT.get_information_signed_block(REQUEST_ID, block_id=None)
    assert isinstance(data, Response)
    assert isinstance(data.payload, BlockHeader)

    # Block ID = Height.
    data: Response = \
        await NODE_BINARY_CLIENT.get_information_signed_block(REQUEST_ID, block_id=18)
    assert isinstance(data, Response)
    assert isinstance(data.payload, BlockHeader)

    # Block ID = Hash.
    data: Response = \
        await NODE_BINARY_CLIENT.get_information_signed_block(REQUEST_ID, block_id=data.payload.parent_hash)
    assert isinstance(data, Response)
    assert isinstance(data.payload, BlockHeader)
