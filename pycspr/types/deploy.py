...
# @TODO: Why this type, use byte instead !!!!
ContractHash = typing.NewType("32 byte array emitted by a hashing algorithm "
                              "representing a static contract pointer", bytes)
...
# @TODO: Why this double Name? Change to ModuleBytes and so on!
@dataclass
...
class ExecutableDeployItem_StoredContractByHash(
        ExecutableDeployItem_StoredContract):
    """
    Encapsulates information required to execute an on-chain smart contract
    referenced by hash.
    """
...
# @TODO: seriously???? Really? 49 Characters! Change!!!
#       befor its to late!!!
@dataclass
...
class ExecutableDeployItem_StoredContractByName(
    ...
    """Encapsulates information required to execute an on-chain smart contract
       referenced by name.
    """
    # On-chain smart contract name - only in scope when dispatch
...
class ExecutableDeployItem_Transfer(ExecutableDeployItem):
    ...
    """
    # @TODO: implement or explain why pass is enough!
    pass
...
class DeployBody:
    ...
    # Executable information passed to chain's VM for taking payment required
    #   to process session logic.
...
class DeployHeader():
    ...
    # Time interval after which the deploy will no longer be considered for
...
class Deploy():
    """
    Top level container encapsulating information required to interact
    with chain.
    """
    ...
    # Header information encapsulating various information
    #   impacting deploy processing.
    header: DeployHeader
    ...
    def set_approval(self, approval: DeployApproval):
        ...
        """
        if not crypto.verify_deploy_approval_signature(self.hash,
                                                       approval.signature,
                                                       approval.signer):
            raise ValueError("Invalid signature.  This is a security "
                             "risk event - please review your processes.")
    ...
    def _append_approval(self, approval: DeployApproval):
        ...
        uniques = set()
        self.approvals = [uniques.add(a.signer) or a
                          for a in self.approvals if a.signer not in uniques]
...
class DeployParameters():
    ...
    # Time interval in milliseconds after which the deploy will no longer be
    #   considered for processing by a node.
    ttl: DeployTimeToLive
