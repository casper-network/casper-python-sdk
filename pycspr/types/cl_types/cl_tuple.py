...
class CL_Type_Tuple1(CL_Type):
    ...
    def as_bytes(self) -> bytes:
        return bytes([self.type_key.value]) + self.t0_type.as_bytes()

    def as_json(self) -> str:
        return {
            "Tuple1": self.t0_type.as_json()
        }

    @staticmethod
    def from_bytes(_: bytes) -> "CL_Type_Tuple1":
...
class CL_Type_Tuple2(CL_Type):
    ...
    def as_bytes(self) -> bytes:
        return bytes([self.type_key.value]) + \
               self.t0_type.as_bytes() + \
               self.t1_type.as_bytes()

    def as_json(self) -> str:
        return {
            "Tuple2": [
                self.t0_type.as_json(),
                self.t1_type.as_json()
            ]
        }

    @staticmethod
    def from_bytes(_: bytes) -> "CL_Type_Tuple2":
        raise NotImplementedError()
    ...
    def as_json(self) -> str:
        return {
            "Tuple3": [
                self.t0_type.as_json(),
                self.t1_type.as_json(),
                self.t2_type.as_json()
            ]
        }

    @staticmethod
    def from_bytes(_: bytes) -> "CL_Type_Tuple3":
        raise NotImplementedError()
