...
def create_private_key(algo: crypto.KeyAlgorithm, pvk: bytes, pbk: bytes
                       ) -> PrivateKey:
...
def create_public_key_from_account_key(account_key: bytes) -> PublicKey:
    ...
    """
    return create_public_key(crypto.KeyAlgorithm(account_key[0]),
                             account_key[1:])


def parse_private_key(fpath: pathlib.Path,
                      algo: typing.Union[str, crypto.KeyAlgorithm]
                      ) -> PrivateKey:
    """Returns on-chain account information deserialised from a secret key held
        on file system.
...
def parse_private_key_bytes(pvk: bytes,
                            algo: typing.Union[str, crypto.KeyAlgorithm]
                            ) -> PrivateKey:
...
def parse_public_key_bytes(pbk: bytes,
                           algo: typing.Union[str, crypto.KeyAlgorithm]
                           ) -> PublicKey:
    """Returns an account holder's public key.
