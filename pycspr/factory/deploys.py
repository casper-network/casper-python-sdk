...
def create_deploy(params: DeployParameters, payment: ExecutableDeployItem,
    ...
    return Deploy(approvals=[], hash=create_digest_of_deploy(header),
                  header=header, payment=payment, session=session)


def create_deploy_approval(deploy: typing.Union[bytes, Deploy],
                           approver: PrivateKey):
    """Returns a deploy approval to be associated with a deploy.
    :param deploy: Either a deploy to be approved, or the hash of a deploy
                   to be approved.
    :param approver: Account key of entity approving a deploy.
    :returns: A deploy approval to be associated with a deploy.
    """
    deploy_hash = deploy.hash if isinstance(deploy, Deploy) else deploy
    # @TODO: Why assert? Throw exception instead!
    assert len(deploy_hash) == 32, "Invalid deploy hash"
    return DeployApproval(
        approver.account_key,
        crypto.get_signature_for_deploy_approval(deploy_hash,
                                                 approver.private_key,
                                                 approver.key_algo)
    )


def create_deploy_argument(name: str, parsed: object,
                           cl_type: typing.Union[CLTypeKey, CLType]
                           ) -> ExecutionArgument:
    """Returns an argument associated with deploy execution
        information (session|payment).
...
def create_deploy_body(payment: ExecutableDeployItem,
                       session: ExecutableDeployItem) -> DeployBody:
...
def create_deploy_header(body: DeployBody, params: DeployParameters
                         ) -> DeployHeader:
...
def create_deploy_parameters(
    ...
    :param dependencies: Array of hashes of deploys that must be processed
                            prior to this one.
    :param gas_price: Array of hashes of deploys that must be processed prior
                        to this one.
    :param timestamp: Milliseconds since epoch when deploy was instantiated.
    :param ttl: Humanized time interval prior to which deploy must
                    be processed.
    """
...
def create_deploy_ttl(humanized_ttl: str = constants.DEFAULT_DEPLOY_TTL
                      ) -> DeployTimeToLive:
    """Returns a deploy's time to live after which it will not longer
        be accepted by a node.
    :param humanized_ttl: A humanized ttl, e.g. 1 day.
    """
    as_milliseconds = conversion.humanized_time_interval_to_milliseconds(
        humanized_ttl
    )
    if as_milliseconds > constants.DEPLOY_TTL_MS_MAX:
        raise ValueError("Invalid deploy ttl "
                         f"{humanized_ttl} = {as_milliseconds} ms.  "
                         f"Maximum (ms) = {constants.DEPLOY_TTL_MS_MAX}")
    return DeployTimeToLive(humanized=humanized_ttl,
                            as_milliseconds=as_milliseconds)


def create_native_transfer(params: DeployParameters, amount: int,
                           target: bytes, correlation_id: int = None,
                           ) -> Deploy:
    """Returns a native transfer deploy.
    :param params: Standard parameters used when creating a deploy.
    :param amount: Amount in motes to be transferred.
    :param target: Target account hash.
    :param correlation_id: An identifier used by dispatcher to subsequently
                            correlate the transfer to internal systems.
    :returns: A native transfer deploy.
    """
    return create_deploy(
        params,
        create_standard_payment(
            constants.STANDARD_PAYMENT_FOR_NATIVE_TRANSFERS
        ),
        create_native_transfer_session(amount, target, correlation_id)
    )


def create_native_transfer_session(amount: int, target: bytes,
    ...
    return ExecutableDeployItem_Transfer(
        args=[
            ...
            create_deploy_argument(
                ...
                create_cl_type_of_option(create_cl_type_of_simple(
                                         CLTypeKey.U64))
            ),
        ]
    )
...
def create_validator_auction_bid(params: DeployParameters, amount: int,
                                 delegation_rate: int,
                                 public_key: PublicKey, path_to_wasm: str
                                 ) -> Deploy:
    """Returns a validator auction bid deploy.
    :param params: Standard parameters used when creating a deploy.
    :param amount: Amount in motes to be submitted as an auction bid.
    :param delegation_rate: Percentage charged to a delegator
    ...
    payment = create_standard_payment(
        constants.STANDARD_PAYMENT_FOR_AUCTION_BID
    )
    # @TODO: simplify!
...
def create_validator_auction_bid_withdrawal(params: DeployParameters,
                                            amount: int,
    ...
    :returns: A standard delegation deploy.
    """
    # @TODO: oh boy! simplify! DRY!
    #           and why is this here in factory module?
    #           why not in deploys???
    payment = create_standard_payment(
...
def create_validator_delegation(
    ...
    """
    payment = create_standard_payment(
        constants.STANDARD_PAYMENT_FOR_DELEGATION
    )
    # @TODO: and again!
    session = ExecutableDeployItem_ModuleBytes(
        module_bytes=_io.read_wasm(path_to_wasm),
