...
WITHDRAW_BID_WASM = pathlib.Path(os.getenv("NCTL")) / "assets" / "net-1" / \
    "bin" / "auction" / "withdraw_bid.wasm",

# CLI argument parser.
_ARGS = argparse.ArgumentParser("Demo illustrating how to unstake CSPR tokens "
                                "as a validator.")
...
#   to NCTL bin/eco/withdraw_bid.wasm.
_ARGS.add_argument(
    "--path-to-wasm",
    default=WITHDRAW_BID_WASM,
    dest="path_to_wasm",
...
def _get_deploy(args: argparse.Namespace, validator: PrivateKey,
                validator_purse: UnforgeableReference) -> Deploy:
    ...
    # Set deploy.
    deploy = factory.create_validator_auction_bid_withdrawal(
