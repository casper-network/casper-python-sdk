...
_ARGS = argparse.ArgumentParser(
    "Demo illustrating how to qeury an ERC-20 smart contract."
)
...
def _get_contract_hash(
    args: argparse.Namespace, client: NodeClient, operator: PrivateKey
) -> bytes:
...
def _get_deploy(
    ...
    session: StoredContractByHash = StoredContractByHash(
        ...
        hash=contract_hash,
        args=[
            pycspr.create_deploy_arg("amount", pycspr.cl_value.u256(args.amount)),
            pycspr.create_deploy_arg("recipient", pycspr.cl_value.public_key(user)),
        ],
    )
