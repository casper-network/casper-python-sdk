...
FAUCET_PUB_KEY_HEX = pathlib.Path(os.getenv("NCTL")) / "assets" / "net-1" / \
    "faucet" / "public_key_hex",

# CLI argument parser.
_ARGS = argparse.ArgumentParser("Demo illustrating how to qeury an ERC-20 "
                                "smart contract.")

# CLI argument: path to contract operator public key - defaults to NCTL faucet.
_ARGS.add_argument(
    "--operator-public-key-path",
    default=FAUCET_PUB_KEY_HEX,
...
def _get_contract_data(client: NodeClient, contract_hash: bytes,
                       key: str) -> bytes:
    """Queries chain for data associated with a contract.  """
    cl_value = client.queries.get_state_item(f"hash-{contract_hash.hex()}",
                                             key)
...
def _get_operator_key(args: argparse.Namespace) -> PublicKey:
    ...
    return factory.parse_public_key(args.path_to_operator_public_key)


def _get_contract_hash(args: argparse.Namespace, client: NodeClient,
                       operator: PrivateKey) -> bytes:
    """Returns on-chain contract identifier. """
    # We query operator account for a named key == ERC20, we then return
    #   the parsed named key value.
    ...
    raise ValueError("ERC-20 has not been installed ... "
                     "see how_tos/how_to_install_a_contract.py")


def _get_deploy(args: argparse.Namespace, contract_hash: bytes,
                operator: PrivateKey, user: PublicKey) -> Deploy:
    """Returns delegation deploy to be dispatched to a node. """
    # Set standard deploy parameters.
    params = factory.create_deploy_parameters(account=operator,
    ...
    # Set session logic.
    session = ExecutableDeployItem_StoredContractByHash(
