...
USER2_PUB_KEY_HEX = pathlib.Path(os.getenv("NCTL")) / "assets" / "net-1" \
...
def _get_counter_parties(args: argparse.Namespace
    ...
    cp1 = factory.parse_private_key(
        args.path_to_cp1_secret_key,
        args.type_of_cp1_secret_key,
    )
    cp2 = factory.parse_public_key(args.path_to_cp2_account_key)
    return cp1, cp2


def _get_deploy(args: argparse.Namespace, cp1: PrivateKey, cp2: PublicKey
                ) -> Deploy:
